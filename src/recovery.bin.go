...
func ReadPublicKey(gpghomedir, name string) string {
	publicKeyData := rplib.Shellexecoutput("gpg", fmt.Sprintf("--homedir=%s", gpghomedir), "-a", "--export", name)
	publicKeyLines := strings.Split(publicKeyData, "\n")
	key := "openpgp " + strings.Join(publicKeyLines[3:len(publicKeyLines)-2], "")
	log.Println("key:", key)
	return key
}

func SignDeviceSerial() {
	var err error

	rplib.Shellexec("/recovery/bin/rngd", "-r", "/dev/urandom")
	err = os.MkdirAll("/tmp/writable/recovery/.gnupg/", 0700)
	rplib.Checkerr(err)
	...
	rplib.Shellexec("gpg", fmt.Sprintf("--homedir=/tmp/writable/recovery/.gnupg"), "--batch", "--gen-key", "/tmp/gen-key-script")

	key := ReadPublicKey("/tmp/writable/recovery/.gnupg", "SERIAL")
	authority := "System"
	brand := "System Inc."
	model := "Router 3400"
	revision := "12"
	...
	vaultServer := rplib.Shellcmdoutput("ip route | awk '/default/ { print $3 }'") // assume identity-vault is hosted on the gateway
	log.Println("vaultServer:", vaultServer)
	r, _ := http.Post(fmt.Sprintf("http://%s:8080/1.0/sign", vaultServer), "application/x-www-form-urlencoded", body)
	response, err := ioutil.ReadAll(r.Body)
	log.Println("Device Sign error:", err)

	err = ioutil.WriteFile("/tmp/writable/recovery/deviceSerial.txt", response, 0600)
	rplib.Checkerr(err)
}
...
func main() {
	...
	rplib.CreateBootEntry(device, recovery_nr, "\\EFI\\BOOT\\BOOTX64.EFI", "factory_restore")
	rplib.CreateBootEntry(device, normal_boot_nr, "\\EFI\\BOOT\\BOOTX64.EFI", "snappy_ubuntu_core")

	if "install" == RECOVERY_TYPE {
		log.Println("[EXECUTE FACTORY INSTALL]")

		// TODO: check criteria
		// set next recoverytype to factory_restore
		...
		rplib.Shellexec("grub-editenv", "/recovery_partition/efi/ubuntu/grub/grubenv", "set", "recoverytype=factory_restore")

		// pass first factory restore confirmation because of factory install mode executed.
		rplib.Shellexec("grub-editenv", "/recovery_partition/efi/ubuntu/grub/grubenv", "set", "firstfactoryrestore=yes")
	}
